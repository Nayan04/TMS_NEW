...
class database
{
	...
	function update_enq_per($enq_id,$fmid,$fmname,$company,$pos,$street,$taluka,$district,$stat,$country,$pincode,$mobile,$phno,$email,$location){
...
project_location='$location' where enquiry_id='$enq_id'"); 
	...
	function updated_time_trace($enq_id,$date,$user_id,$times){
		...
		$sql = sprintf("update enquiry_tracing set updated_date='$date',update_time='$times',updated_by='$user_id' where enquiry_id='$enq_id'"); 
		echo $sql;
		            $rs=mysql_query($sql,$this->link);
		            if(!$rs){
			         echo mysql_error($this->link);
