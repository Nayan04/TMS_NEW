...
class main_database {
    ...
    function get_enquiry_by_status($status, $enq_id) {

        $sql = sprintf("select * from inspection_records where is_live=1 and status='$status' and enquiry_id='$enq_id' and  isactive=1 and is_inspection_done=1");
        $rs = mysql_query($sql, $this->link);
        if (!$rs) {
            echo mysql_error($this->link);
        } else {
